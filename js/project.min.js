...
var $pickerLib = $(".ui-picker-lib"), pickerMap, pickerMarker;
"undefined" != typeof google && $(".ui-picker-map-wrap").length && initPickerMap(),
"undefined" != typeof jQuery.ui && ($(".ui-picker-draggable-handler").draggable({
    addClasses: !1,
    appendTo: "body",
    cursor: "move",
    cursorAt: {
...
$pickerLib.selectable({
    ...
    selecting: function(a, b) {
        ...
        c.addClass("tile-brand-accent ui-picker-selected"),
        $(".ui-picker-info").addClass("ui-picker-info-active").removeClass("ui-picker-info-null"),
        $(".ui-picker-info-desc-wrap").html(c.find(".ui-picker-info-desc").html()),
        $(".ui-picker-info-title-wrap").html(c.find(".ui-picker-info-title").html());
        var d = new google.maps.LatLng(c.find(".ui-picker-map-lat").html(),c.find(".ui-picker-map-lng").html());
        pickerMap.setCenter(d),
        pickerMarker.setMap(pickerMap),
        pickerMarker.setPosition(d)
    },
    unselecting: function(a, b) {
        var c = $(b.unselecting).parent();
        if (c.removeClass("tile-brand-accent ui-picker-selected"),
        $(".ui-picker-selected").length) {
            var d = $($(".ui-picker-selected")[0]);
            $(".ui-picker-info-desc-wrap").html(d.find(".ui-picker-info-desc").html()),
            $(".ui-picker-info-title-wrap").html(d.find(".ui-picker-info-title").html());
            var e = new google.maps.LatLng(d.find(".ui-picker-map-lat").html(),d.find(".ui-picker-map-lng").html());
            pickerMap.setCenter(e),
            pickerMarker.setMap(pickerMap),
            pickerMarker.setPosition(e)
        } else
...
$("#ui_datepicker_example_3").pickdate({
    disable: [[2016, 0, 12], [2016, 0, 13], [2016, 0, 14]],
    today: ""
}),
...
$("#ui_el_loading_example_wrap .tile-active-show").each(function(a) {
    ...
    c.on("show.bs.tile", function(a) {
        $(".el-loading", c).hasClass("el-loading-done") || (b = setTimeout(function() {
            $(".el-loading", c).addClass("el-loading-done"),
            c.prepend('<div class="tile-sub"><p>Additional information<br><small>Aliquam in pharetra leo. In congue, massa sed elementum dictum, justo quam efficitur risus, in posuere mi orci ultrices diam.</small></p></div>')
        }, 6e3))
    })
});
var snackbarText = 1;
...
$("#dove_snackbar").on("click", function() {
	$("body").snackbar({
		content: '<div class="snackbar-text">成功添加 德芙巧克力（盒装） 到购物车</div><div class="pull-right"><a data-dismiss="snackbar" style="color: #1976D2;">撤销</a></div>',
		show: function() {
			//
		}
	})
}),
$("#chair_snackbar").on("click", function() {
	$("body").snackbar({
		content: '<div class="snackbar-text">成功添加 小板凳 到购物车</div><a data-dismiss="snackbar" style="color: #1976D2;">撤销</a>',
		show: function() {
			//
		}
	})
}),
$("#bottle_snackbar").on("click", function() {
	$("body").snackbar({
		content: '<div class="snackbar-text">成功添加 奶壶壶 到购物车</div><a data-dismiss="snackbar" style="color: #1976D2;">撤销</a>',
		show: function() {
			//
		}
	})
